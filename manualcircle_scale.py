...
def calibrate_gauge(gauge_number, file_type):
    ...
    x,y=350,378 #hardcoded values, fetch from db
    ...
    cv2.imwrite('manualcircle.jpg', img)
...
def get_current_value(img, min_angle, max_angle, min_value, max_value, x, y, r, gauge_number, file_type):
    ...
    # find lines
    minLineLength = 50#was 10
    maxLineGap = 100#was 50
    ...
    cv2.imwrite('manualline.jpg', img)
...
def main():
    ...
    #mqtt(val)
    sendToSQL(val,server,username,password,database)
...
def sendToSQL(val,server,username,password,database): 
...
def mqtt(val):
    # asyncio.run(main())
    asyncio.get_event_loop().run_until_complete(mqttscale.main(val))
